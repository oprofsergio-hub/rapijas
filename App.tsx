...
const TeachersPanel = ({ 
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 className="text-lg font-semibold mb-4 text-gray-800">Gerenciar Professores</h3>
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
              <select 
                 ...
                 className="border rounded-lg px-3 py-2 text-sm bg-purple-50 border-purple-200 text-purple-900"
              >
         ...
         <div className="mt-4 p-4 bg-yellow-50 text-yellow-800 text-sm rounded-lg border border-yellow-200">
           ...
           <strong>Atenção:</strong> Cada modalidade (EJA, Fundamental, etc.) possui seu próprio calendário independente. Certifique-se de estar editando o calendário correto no menu superior.
         </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 overflow-x-auto">
        ...
        <div className="mt-4 p-4 bg-blue-50 text-blue-800 text-sm rounded-lg border border-blue-200">
           <strong>Multidisciplinar / Multiseriado:</strong> Para turmas onde o professor dá aula para várias séries ao mesmo tempo (ex: 2º e 3º ano), cadastre a turma como "Multiseriada" e atribua o professor na grade acima. O RAPI calculará as horas baseadas no horário do professor.
         </div>
      </div>
    </div>
  );
...
const ReportsPanel = ({ state }: { state: AppState }) => {
  ...
  const [view, setView] = useState<'detailed' | 'teacher_summary'>('teacher_summary');

  const data = useMemo(() => calculateRapi(state, month, view), [state, month, view]);
  ...
  const downloadCSV = () => {
     ...
     link.setAttribute("download", `rapi_${month}_${view}.csv`);
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-wrap gap-4 items-center justify-between">
              <div className="flex gap-4 items-center">
                  ...
                  <div>
                      ...
                      >
                          <option value="teacher_summary">Resumo por Professor</option>
                          <option value="detailed">Detalhado (Turma/Disc)</option>
                      </select>
                  </div>
              </div>
              <button onClick={downloadCSV} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 flex items-center gap-2">
                  <Icon name="fa-file-excel" /> Exportar CSV
              </button>
          </div>
              ...
              <table className="w-full text-left">
                  <thead className="bg-brand-50 border-b border-brand-100">
                      <tr>
                          ...
                          {view === 'detailed' && <th className="p-3 text-sm font-semibold text-brand-800">Turma</th>}
                          {view === 'detailed' && <th className="p-3 text-sm font-semibold text-brand-800">Disciplina</th>}
                  ...
                  <tbody className="divide-y">
                      {data.map((row, idx) => (
                          <tr key={idx} className="hover:bg-gray-50">
                              ...
                              {view === 'detailed' && <td className="p-3 text-sm">{row.className}</td>}
                              {view === 'detailed' && <td className="p-3 text-sm">{row.subject}</td>}
                      ...
                      {data.length === 0 && <tr><td colSpan={7} className="p-8 text-center text-gray-500">Nenhum dado calculado para este período. Verifique o calendário e a grade.</td></tr>}
                  </tbody>
              </table>
          </div>
      </div>
